...
<%!
    ...
    String badgeClass(String status) {
        if (status == null) {
            return "ep-status-menunggu";
        }
        switch (status) {
            case "Diproses":
                return "ep-status-diproses";
            case "Dibayar":
                return "ep-status-dibayar";
            case "Dikirim":
                return "ep-status-dikirim";
            case "Selesai":
                return "ep-status-selesai";
            default:
                return "ep-status-menunggu";
        }
    }
...
<%
    ...
    String[] tabs = {"Semua", "Menunggu Pembayaran", "Dibayar", "Diproses", "Dikirim", "Selesai"};
    ...
    <body>
        ...
        <main class="container py-4">
            ...
            <div class="ep-order-card">
                ...
                <!-- Ringkasan produk -->
                <div class="ep-order-products">
                    <%= esc(ringkasProduk(items))%>
                </div>
                ...
                <div class="ep-order-foot">
                    ...
                    <div class="ep-order-actions">
                        ...
                        <% } else if ("Selesai".equals(status)) {
                            ...
                            if (!reviewed) {
                        %>
                        <a href="<%= ctx%>/buyer/reviews?add&transaksi_id=<%= t.getId()%>" class="ep-btn-order">
                            <i class="bi bi-star-fill text-warning me-1"></i> Review
                        </a>
                        <% } else { %>
                        <span class="text-muted small"><i class="bi bi-check-circle-fill text-success"></i> Sudah Direview</span>
                        <% } %>
                        <% } else if ("Dikirim".equals(status)) {%>
                        <form method="post" action="<%= ctx%>/buyer/orders" style="display:inline;">
                            <input type="hidden" name="action" value="complete">
                            <input type="hidden" name="id" value="<%= t.getId()%>">
                            <button type="submit" class="ep-btn-order">Terima Barang</button>
                        </form>
                        <% } else {%>
                        <a href="<%= ctx%>/buyer/payment?id=<%= t.getId()%>" class="ep-btn-order">
                            Lihat Detail
